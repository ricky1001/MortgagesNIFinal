...
quickCallHelpers = JSON.parse(quickCallHelpers)
...
quickCallHelpers.adjustedTimes.forEach(function(item){
...
function createList(){
    ...
    if(hoursPastNine > 1){
        for(var i = 0; i < adjustedTimes.length; i++){
            let currentItem = `<option class="pickTimeOption" value="${adjustedTimes[i]}">${adjustedTimes[i]}</option>`;
